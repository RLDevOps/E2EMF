...
//		console.log(searchCheck);
		...
		if(!searchCheck){ // check if not search form submitted
		// parse url to jquery
		loadURL(url, container, '');
		}
		searchCheck = false;
...
// If header search form submitted
$('#frm-header-search').submit(function(){
	
	var searchterm = $('#search-fld').val(); // get the search field value
	var frmAction = $(this).attr('action'); // get the form action
	container = $('#content'); // container area
	searchCheck = true; // Set searchCheck true to prevent window hashchange event
//	console.log("Hi:"+searchCheck);
	window.location.hash = '#'+frmAction; // add formaction in window location hash
	loadURL(frmAction, container, searchterm); // Load the url with searchterm
//	
	 // Set searchCheck back to false
	return false; //prevent form default submit behaviour 
})

// LOAD AJAX PAGES

function loadURL(url, container, serachterm) {
...
function drawBreadCrumb() {
	...
	$("#ribbon ol.breadcrumb").append($("<li><a href='#ajax/e2e_canada_flow.html'>Home</a></li>"));
	var activeNav = $('nav li.active > a');
	var len = activeNav.length;
	activeNav.each(function(index,element) {
		var navLink = $(this).attr('href');
		var navText = $.trim($(this).clone().children(".badge").remove().end().text());
		if (index == len - 1 || navLink == '#') {
			$("#ribbon ol.breadcrumb").append($("<li></li>").html(navText));
		}
		else{
			$("#ribbon ol.breadcrumb").append($("<li></li>").html("<a href='#" + navLink  +"'>"+navText + "</a>"));
		}
	});
...
function  loadingspinner(){
 ...
 var opts = {
   ...
   // var target = document.querySelectorAll("loading-1, loading-2")
    var target1 = document.getElementById('loading-1');
    var target2 = document.getElementById('loading-2');
    var target3 = document.getElementById('loading-3');
    var target4 = document.getElementById('loading-4');
    var target5 = document.getElementById('loading-5');
    var target6 = document.getElementById('loading-6');
    var target7 = document.getElementById('loading-7');
    var target8 = document.getElementById('loading-8');
    var target9 = document.getElementById('loading-9');
    var target10 = document.getElementById('loading-10');
    var target11 = document.getElementById('loading-11');
    ...
    var spinner = new Spinner(opts).spin(target6);
    var spinner = new Spinner(opts).spin(target7);
    var spinner = new Spinner(opts).spin(target8);
    var spinner = new Spinner(opts).spin(target9);
    var spinner = new Spinner(opts).spin(target10);
 ...
 $("#login-form").submit(function() {
                 ...
                 success: function (dataR) {                     
                     ...
                     $.each( data, function( key, value ) {
                         ...
                         if (usn == un && psw == pw) {
                             ...
                             sessionStorage.setItem("user", usn);
        ...
        if (authenticationPass) {
            ...
            $.SmartMessageBox({
			title : "<i class='fa fa-exclamation-triangle'></i>&nbsp;<span class='txt-color-red'>ERROR !!</span>",
			content : "Please Enter The Valid Username and Password.",
			buttons : '[close]'
